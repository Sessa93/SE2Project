...
\subsection{SSIServlet.java}
  ...
  \begin{itemize}
    ...
    \item \textbf{\textit{requestHandler()}}
      ...
      In this block of code the method checks if the url associated with the given request does not exists or points to a resource in the `WEB-INF` or `META-INF` subdirectories,
      ...
      In this block of code the method tries to retrieve the resource associated with the url of the given request.
      ...
      it sets the mime type, the characters encoding of the output text and the expiration time(in seconds, see init()).
    ...
    \item \textbf{\textit{processSSI()}}
      ...
      in a given resource and checks the characters encoding of this data according to the configuration of the Servelet
